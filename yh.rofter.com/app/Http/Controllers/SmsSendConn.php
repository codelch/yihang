...
class SmsSendConn
{
     ...
     * $pwd：用户密码
     */
    public function encryptPwd($userid, $pwd)
    {
        try {
            $char = '00000000'; //固定字符串
    ...
    public function singleSend($data)
    {
        try {
            ...
            $encrypt           = $this->encryptPwd($data['userid'], $data['pwd']); //密码进行MD5加密
            $data['pwd']       = $encrypt['pwd']; //获取MD5加密后的密码
            $data['timestamp'] = $encrypt['time']; //获取加密时间戳
            $data['content']   = $this->encryptContent($data['content']); //短信内容进行urlencode加密
    ...
    public function multiSend($data)
    {
        try {
            ...
            foreach ($data['multimt'] as $k => $v) {
                $data['multimt'][$k]['content'] = $this->encryptContent($v['content']); //每一条个性化的短信内容进行urlencode加密
            }
            $post_data = json_encode($data); //将数组转化为JSON格式
